...
const Navbar = () => {
  ...
  const nav_links = [
    { name: "Home", link: "/" },
    { name: "About", link: "/about-er-summit" },
    { name: "Executive Panel", link: "/executive-panel-members/" },
    { name: "Tracks", link: "/er-summit-conference-tracks/" },
    { name: "Orators", link: "/er-summit-orators" },
    { name: "Schedule", link: "/er-summit-event-schedule" },
        ...
        >
          {nav_links.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className={`hover-underline-animation transition border-b-2 ${location.pathname === item.link
                  ? "border-one text-one font-bold"
                  : "border-transparent text-gray-700 hover:text-one"
                }`}
              style={{
                marginRight: scrolled ? "0px" : "4px",
              }}
